...
**Bedobás (concede):** the declarer may **throw in** at any point once play has begun (even before the opening lead) via a *Bedobom* button. The hand ends immediately, the defenders win, and **every component is scored as a loss** using whatever **kontra** levels were reached before the concede (a lost Ulti still doubles). The **2-card required-ulti premium and the kötelező saying acknowledgement both still stand** on a concede (they reward the committed félkez bid, not the play). `applyConcede` sets `play.conceded`; `calculateRoundScore({ conceded:true })` forces every component lost (no parti card-race / 100 bonus) but still adds the `ulti_bonus`; `_markKotelezo` runs at hand end as usual. Client: `play:concede` from the *Bedobom* button in `ClaimBar` (with a one-step confirm).
