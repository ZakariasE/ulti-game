...
- **Parti contract, before any card is played** (`!play.openingLeadDone`): *Bedobom* concedes immediately as a plain loss — **no negotiation** (there is nothing to play for yet). `startConcede` short-circuits to `applyConcede(hundred:false)` and returns `{resolved:true}` (`concede:start` → `round:completed`). The client sends `play:concede` directly here (its `negotiate` guard = `hasParti && !beforeAnyCard`).
- **Parti contract, once play has begun:** *Bedobom* instead opens a **negotiation** (still no card played this turn) — `applyConcede` is never called unilaterally here:
